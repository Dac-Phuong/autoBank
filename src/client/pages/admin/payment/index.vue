<template>
  <UiContent title="Payment" sub="Quản lý giao dịch nạp tiền">
    ...
    <UCard :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #code-data="{ row }">
          <UBadge variant="soft" @click="viewPayment(row.gate._id, row.code, row.money)" color="green" class="cursor-pointer">
            {{ row.code || 'No Code' }}
          </UBadge>
        </template>
    ...
    <UModal v-model="modal.payment">
      <ServiceGateView :state="statePayment" />
    </UModal>
...
const viewPayment = (_id, code, money) => {
  statePayment.value = {
    _id,
    code,
    money
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('admin/payment/list', JSON.parse(JSON.stringify(page.value)))
...
const successAction = async () => {
  try {
    ...
    await useAPI('admin/payment/verify', JSON.parse(JSON.stringify(stateSuccess.value)))
...
const refuseAction = async () => {
  try {
    ...
    await useAPI('admin/payment/verify', JSON.parse(JSON.stringify(stateRefuse.value)))
...
const waitingAction = async () => {
  try {
    ...
    await useAPI('admin/payment/verify', JSON.parse(JSON.stringify(stateWaiting.value)))
