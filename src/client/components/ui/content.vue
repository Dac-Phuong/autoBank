<template>
  <div class="UiContent">
    <UiFlex class="UiContentHeader mb-4">
      ...
      <div class="grow">
        <UiFlex class="gap-4">
          <UiText weight="semibold" :size="size">{{ title || '...' }}</UiText>
        ...
        <UiFlex v-if="!!sub || !!slots['sub']">
          ...
          <div>
            ...
            <UiText class="text-xs lg:text-sm inline cursor-pointer" color="primary" v-if="!!sub && sub.length > 100"
              @click="showMore = !showMore">{{ !showMore ? 'Xem thêm' : 'Ẩn' }}</UiText>
          </div>
        </UiFlex>
      </div>
    </UiFlex>
